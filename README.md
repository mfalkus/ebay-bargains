...
**Category lists (required for the category dropdown)**  
The category dropdown loads its list from static JSON files so the server never parses the ~2.5MB tree per request. Generate them once (and after eBay taxonomy updates):

```bash
# From project root; requires .env with EBAY_CLIENT_ID and EBAY_CLIENT_SECRET
php scripts/build-categories.php
```

This fetches category trees for eBay UK and US and writes `public/data/categories_EBAY_GB.json` and `public/data/categories_EBAY_US.json`. The frontend requests the file for the selected marketplace.

**Front-end assets (optional)**  
JS/CSS (Tom Select) are served from `public/vendor/`. To refresh from npm:
...
This copies `tom-select.css` and `tom-select.complete.min.js` from `node_modules` into `public/vendor/`. If `public/vendor/` is already populated (e.g. committed), you can run the app without Node/npm.
...
- `public/data/` – Static category JSON files (generated by `scripts/build-categories.php`).
- `scripts/build-categories.php` – Build step: fetches category trees and writes `public/data/categories_*.json`.
- `src/EbayApi.php` – eBay OAuth (client credentials) + Browse API `item_summary/search`.
- `src/CategoryTreeLoader.php` – Used only by the build script to fetch and flatten category trees.
